...
$teleg_login = ';{:local mac $"mac-address";:local Mwp [/ip hotspot user get [find name="$user"] uptime];:local Mpkt [/ip hotspot user get [find name="$user"] profile];:local Md [/ip hotspot user get [find name="$user"] bytes-in];:local Mu [/ip hotspot user get [find name="$user"] bytes-out];:local Mt [((($Md)+($Mu))/1048576)];:local Ma [/ip hotspot active print count-only];/tool fetch url="https://api.telegram.org/bot'.$_BOTAPI.'/sendMessage?chat_id='.$_CHATID.'&text====>> Masuk $user %0A- IP Address           : $address%0A- Mac Address       : $mac%0A- Paket                     : $Mpkt%0A- Waktu Terpakai : $Mwp%0A- Kuota Terpakai  : $Mt Mb%0A- Users Online       : $Ma Users" mode=http keep-result=no}';
$teleg_logout = '{:local mac $"mac-address";:local Mwp [/ip hotspot user get [find name="$user"] uptime];:local Mpkt [/ip hotspot user get [find name="$user"] profile];:local Md [/ip hotspot user get [find name="$user"] bytes-in];:local Mu [/ip hotspot user get [find name="$user"] bytes-out];:local Mt [((($Md)+($Mu))/1048576)];:local Ma [/ip hotspot active print count-only];/tool fetch url="https://api.telegram.org/bot'.$_BOTAPI.'/sendMessage?chat_id='.$_CHATID.'&text=<<=== Keluar $user %0A- IP Address           : $address%0A- Mac Address       : $mac%0A- Paket                     : $Mpkt%0A- Waktu Terpakai : $Mwp%0A- Kuota Terpakai  : $Mt Mb%0A- Users Online       : $Ma Users" mode=http keep-result=no}';
}else{
$teleg_login = '';
$teleg_logout = '';
}
...
$onlogin3 = ':put (",remc,'.$price.','.$validity.','.$graceperiod.','.$timelimit.','.$getlock.',")'.$teleg_login.'; {:local price ('.$price.');:local date [/system clock get date ];:local time [/system clock get time ];:local uptime ('.$validity.');[/system scheduler add disabled=no interval=$uptime name=$user on-event="[/ip hotspot active remove [find where user=$user]];[/ip hotspot user set limit-uptime=1s [find where name=$user]];[/sys sch re [find where name=$user]];[/sys script run [find where name=$user]];[/sys script re [find where name=$user]]" start-date=$date start-time=$time];[/system script add name=$user source=":local date [/system clock get date ];:local time [/system clock get time ];:local uptime ('.$graceperiod.');[/system scheduler add disabled=no interval=\$uptime name=$user on-event= \"[/ip hotspot user remove [find where name=$user]];[/ip hotspot active remove [find where user=$user]];[/sys sch re [find where name=$user]]\"]"];:local bln [:pick $date 0 3]; :local thn [:pick $date 7 11];[:local mac $"mac-address";:local Comment [/ip hotspot user get [find name="$user"] comment]; /system script add name="$date-|-$time-|-$user-|-$price-|-$address-|-$mac-|-'.$validity.'-|-$Comment" owner="$bln$thn" source=$date comment=MIKMOScms]'.$lock.'}'; 
$onlogin4 = ':put (",ntfc,'.$price.','.$validity.',,'.$timelimit.','.$getlock.',")'.$teleg_login.'; {:local price ('.$price.');:local date [/system clock get date ];:local time [/system clock get time ];:local uptime ('.$validity.');[/system scheduler add disabled=no interval=$uptime name=$user on-event= "[/ip hotspot user set limit-uptime=1s [find where name=$user]];[/ip hotspot active remove [find where user=$user]];[/sys sch re [find where name=$user]]" start-date=$date start-time=$time];:local bln [:pick $date 0 3]; :local thn [:pick $date 7 11];[:local mac $"mac-address";:local Comment [/ip hotspot user get [find name="$user"] comment]; /system script add name="$date-|-$time-|-$user-|-$price-|-$address-|-$mac-|-'.$validity.'-|-$Comment" owner="$bln$thn" source=$date comment=MIKMOScms]'.$lock.'}';
